...
//    printf("\n");
    ...
    NSString *str = [NSHomeDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"Documents/1"]];
    ...
    [cav show];
    //DownThread1((__bridge void*)cav,@[@"1"]);
    
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        TransferFilesToPos((__bridge void*)cav,@[@"1"]);
        //DownThread1((__bridge void*)cav,@[@"1"]);
...
- (IBAction)scan:(UIButton *)sender {
    ...
    ScanController1 *sc1  = [[ScanController1 alloc]init];
...
-(void)recvMiniPosSDKStatus{
    ...
    if ([self.statusStr isEqualToString:@"获取参数成功"]) {
        ...
        //[self performSelector:@selector(bindNewDevice) withObject:nil afterDelay:0.2];
        [self performSelector:@selector(verifyDevice) withObject:nil afterDelay:0.2];
    }
...
-(void)verifyDevice{
    ...
    [self verifyParamsSuccess:^{
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"更新成功，准备开门营业" delegate:self cancelButtonTitle:@"确认" otherButtonTitles:@"11", nil];
