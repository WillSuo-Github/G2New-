...
-(void)recvMiniPosSDKStatus{
    ...
    if ([self.statusStr isEqualToString:[NSString stringWithFormat:@"消费失败"]]) {
        ...
        NSString *result = [UIUtils GB2312_To_UTF8:MiniPosSDKGetSaleTradeFailInfo()];
        NSLog(@"消费失败:%@",result);
        
        [self.navigationController popToViewController:self.delegate animated:YES];
        
        if ([self.delegate respondsToSelector:@selector(didFinishSwipingCardWithResult:)]) {

            [self.delegate didFinishSwipingCardWithResult:result];
        }
