...
- (void)viewDidLoad {
    ...
    if (self.dataManager ==nil) {
       self.dataManager = [[NDLThirdPayDataManager alloc]init];
    }
...
-(void)getPayment
{
    [self.dataManager requestPaymentMethodBlock:^(id responseObject, NSString *errorMessage) {
        if (responseObject != nil) {
            self.PayTypeArr = [PayType mj_objectArrayWithKeyValuesArray:responseObject];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
  ...
  /*
   ...
   // cell.textLabel.text =[NSString stringWithFormat:@"number:%ld",indexPath.row];
    ...
    NSLog(@"payment method :%@",payType.mj_keyValues);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"did select paymentName :%@   cptId :%@  paymentType :%@",payType.paymentName,payType.cptId,payType.paymentType);
