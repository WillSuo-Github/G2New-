...
#import "Common.h"
#import "SwipingCardProtocol.h"
@interface ShouQuJinEController ()<DXKeyboardDelegate,SwipingCardProtocol>
...
@property (strong, nonatomic) IBOutlet UIView *wrongView; //错误显示视图
@property (strong, nonatomic) IBOutlet UITextView *wrongTextView; //
...
//    NSLog(@"___%@",self.count);
    ...
    self.wrongView.layer.borderWidth =1;
    self.wrongView.layer.borderColor = [[UIColor blackColor] CGColor];
...
#pragma mark -- SwipingCardProtocol
-(void)didFinishSwipingCardWithResult:(NSString *)result{
    

    NSString *str = [NSString stringWithFormat:@"由于银行返回\"%@\"，因而未能刷卡成功，请顾客更换银行卡重试",result];
    
    self.wrongTextView.text = str;
