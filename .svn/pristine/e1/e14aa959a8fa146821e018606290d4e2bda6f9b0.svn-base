...
- (void)loadHistory{
    ...
    self.area.text = [[NSUserDefaults standardUserDefaults] objectForKey:qiyeSuozaidiqu];
    NSLog(@"!!!%@",[[NSUserDefaults standardUserDefaults] objectForKey:qiyeSuozaidiqu]);
    self.address.text = [KUserDefault objectForKey:qiyeJingyingdizhi];
...
-(void)toolBarDoneBtnHaveClicked:(WDPickView *)pickView resultString:(NSString *)resultString{
    ...
    [KUserDefault setValue:self.areaCode forKey:qiyeareaCode];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField{
    ...
    if (textField.tag == 1) {
        [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:qiyeSuozaidiqu];
    }else if (textField.tag == 2){
        
        [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:qiyeJingyingdizhi];
    }
    [[NSUserDefaults standardUserDefaults] synchronize];
    return YES;
}
