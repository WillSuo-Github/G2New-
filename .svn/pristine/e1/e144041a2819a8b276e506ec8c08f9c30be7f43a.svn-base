...
- (void)setXLabels:(NSArray *)xLabels
{
    ...
    if (_showLabel) {
        xLabelWidth = _chartCavanWidth / [xLabels count] - 3;
    } else {
        xLabelWidth = (self.frame.size.width - _chartMarginLeft - _chartMarginRight) / [xLabels count] - 3;
    }
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                //jet text display text LiuJQ
                if (chartData.showPointLabel == YES&&chartData.maxValue>0) {
                    if (chartData.maxValuePostion>=0&&chartData.maxValuePostion==i) {
            ...
            // Square point
                ...
                // text display text LiuJQ
                if (chartData.showPointLabel == YES) {
                    if (chartData.maxValuePostion>=0&&(chartData.maxValuePostion==i||chartData.maxValue>NSIntegerMin)) {
...
- (void)setupDefaultValues
{
    ...
    self.userInteractionEnabled =  NO;// YES;
...
-(CATextLayer*) createPointLabelFor:(CGFloat)grade pointCenter:(CGPoint)pointCenter width:(CGFloat)width withChartData:(PNLineChartData*)chartData
{
    ...
    [textLayer setBackgroundColor:[[[UIColor grayColor] colorWithAlphaComponent:0.8] CGColor]];
    ...
    [textLayer setString:chartData.maxValue];
