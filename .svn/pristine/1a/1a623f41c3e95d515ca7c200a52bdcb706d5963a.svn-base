...
//    sprintf(data,"%s",test);
    ...
    int _type;
    NSString *operatedType = [[NSUserDefaults standardUserDefaults] stringForKey:@"operatedType"];
    _type=[operatedType intValue];
        ...
        if (_type == 1 || _type == 2) {
            ...
            for (int i = 0; i <= datalen; i++) {
                if (BreakupRecvPack(data[i]) == 0) {
                    NSLog(@"scc");
                    _type =0;
                    [[NSUserDefaults standardUserDefaults] setValue:@"0" forKey:@"operatedType"];
                    char bankCardStr[22]="";
                    getBankCardNo(bankCardStr);
