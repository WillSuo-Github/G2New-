...
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:btn];
    ...
    if (self.zhifuchuandi.count.length) {
        if (self.zhifuchuandi.count.floatValue !=0.0)
        {
            //zhifuchuandi.count.floatValue - self.zhifuchuandi.RealCount.floatValue
            UIButton *titleButton = [[UIButton alloc]initWithFrame:CGRectMake((self.view.width - 100)/2, 10, 100, 80)];
            [btn sizeToFit];
            
            
            //start of line
            // 现在将找零  修改为  显示实际收取金额
            [titleButton setTitle:[NSString stringWithFormat:@"找零%.2f元",self.zhifuchuandi.count.floatValue] forState:UIControlStateNormal];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if (![self.zhifuchuandi.paymentType isEqualToString:@"1"]) {
        ...
        double count =  self.zhifuchuandi.count.floatValue;
        if (MiniPosSDKDeviceState() == 0) {
   ...
   [[NSNotificationCenter defaultCenter] postNotificationName:KzhifuSuccess object:nil userInfo:self.zhifuchuandi.PayType];
    ...
    if ([self.zhifuchuandi.paymentType isEqualToString:@"1"]) {
        ...
        if (self.zhifuchuandi.billID.length) {
            NSString *urlStr=[NSString stringWithFormat:@"%@/canyin-frontdesk/bill/pop/paymentType/update?billId=%@&cptId=&paymentType=%@&money=%@&dbpId=",ceshiIP,self.zhifuchuandi.billID,self.zhifuchuandi.paymentType,self.zhifuchuandi.RealCount];
            DLog(@"=========%@",urlStr);
