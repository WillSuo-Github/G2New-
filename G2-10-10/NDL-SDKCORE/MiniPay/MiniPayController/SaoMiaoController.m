...
@interface SaoMiaoController ()<AVCaptureMetadataOutputObjectsDelegate,UIAlertViewDelegate>
{
    NSTimer *_timer;
...
@property (nonatomic,strong)XiaoFeiSuccessController *xiaoFeisuccessVC;
...
-(void)requestServerWECHATInfo
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@/canyin-frontdesk/run/barPay/savePaymement2?cpt_id=%@&card_id=%@&momey=%@&auth_code=%@&subject=%@&returnJson=json",ceshiIP,self.zhifuchuandi.cptID,self.zhifuchuandi.VIPforwardUrl,self.zhifuchuandi.count,self.zhifuchuandi.QRCodeValueStr,@"VIPRecharge"];
    NSLog(@"Vip WECHAT recharge :%@",urlStr);
    
    self.promptConsumerSuccessViewController = [[PromptConsumerSuccessViewController alloc]init];
    //self.xiaoFeisuccessVC = [[XiaoFeiSuccessController alloc]init];
    ...
    //self.xiaoFeisuccessVC.zhifuchuandi = zhiFuChuanDiPG;
    ...
    [HttpTool GET:urlStr parameters:nil success:^(id responseObject) {
       // NSLog(@"Vip WECHAT  recharge response :%@",responseObject);
        ...
        NSString *responseResultStr = [responseObject objectForKey:@"result_code"];
        if ([responseResultStr isEqualToString:@"SUCCESS"])
        {
            ...
            //[self.navigationController pushViewController:self.xiaoFeisuccessVC animated:YES];
...
// VIP  http://127.0.0.1:8080/canyin-frontdesk/member/chongzhi/create
-(void)VIPRechargeServerOrder
{
    ...
    NSLog(@"dhjsjdhsd++%@",self.zhifuchuandi.count);
    self.xiaoFeisuccessVC = [[XiaoFeiSuccessController alloc]init];
    ZhiFuChuanDiPG *zhiFuChuanDipg = [[ZhiFuChuanDiPG alloc]init];
